...
for Index = 1: epochs
    ...
    if new_tracking
        ...
        elseif enable_EKF 
            if (Index == 1)
                ...
                %    drv_fi_ss2(:,drv_idx)=(fi_ss(:,drv_idx)-LS_G(:,L_ekf))./((drv_idx-1)*(1/L_ekf));
                %end

                drv_fi_ss2(1,:)=zeros(1,L_ekf+1);
                for drv_idx=2:P_ekf
                    drv_fi_ss2(drv_idx,:)=(fi_ss(drv_idx,:)-fi_ss(1,:))./((drv_idx-1)*(1/L_ekf));
                end
                drv_fi_ss2(P_ekf,:)=zeros(1,L_ekf+1);
            end
            % prediction
            X_k=A*X_k;
            P_k=A*P_k*A'+B;
    ...
    if new_tracking
        ...
        %code_output= (1-0.1)*t_los+(0.1)*last_code_output;
