...
enable_NLMS = 0;
enable_PF = 1;
if enable_LS || enable_LMS || enable_NLMS || enable_PF
    new_tracking = 1;
else
    new_tracking = 0;
end
        ...
        elseif enable_PF
            ...
            for idx=1:n_part
                ...
                if Index>1
                    ...
                    t = truncate(pd,-0.5+particle_pred(idx,1),0.5+particle_pred(idx,1));
            ...
            for idx=1:n_part
                for subindex=0:M_pf-1
                    C_s(Index,subindex+1) = C_s(Index,subindex+1) + 
                    weight(idx)*(   (particle(idx,subindex*2+1:subindex*2+2)-x_est_bpf(Index,subindex*2+1:subindex*2+2))*
                                    (particle(idx,subindex*2+1:subindex*2+2)-x_est_bpf(Index,subindex*2+1:subindex*2+2))');
                end
                C_s(Index,subindex+1)
            end
