function [TckResultCT, navSolutionsCT] = trackingCT_POS(file,signal,track,cmn, Acquired,cnslxyz,eph,sbf)
    ...
    rmse = zeros(1,corr_per_chip_plot*2+1);
    ...
    for msIndex = 1: 2000/pdi 
        ...
        for svindex = 1 :svlength
            ...
            if enable_LS & (msIndex >= 14) & (svindex == 1) & (P_i>0)
                ...
                LS_G = zeros(numSample*(msIndex-13), (corr_per_chip_plot*2)+1);
                for subindex = 1: (corr_per_chip_plot*2)+1%remove
                    LS_G(1:1:numSample, subindex) = code_replicas_plot(subindex,:);
                    if msIndex == 15
                        LS_G(numSample+1:1:2*numSample, subindex) = code_replicas_plot(subindex,:);
                    end
                    if msIndex == 16
                        LS_G(numSample*2+1:1:3*numSample, subindex) = code_replicas_plot(subindex,:);
                    end
                end
                if msIndex == 14
                    LS_D = InphaseSignal';
                elseif msIndex == 15
                    LS_D(1:1:numSample) = InphaseSignal';
                    LS_D(numSample+1:1:2*numSample) = last_LS_D;
                    last_last_LS_D = last_LS_D;
                else
                    LS_D(1:1:numSample) = InphaseSignal';
                    LS_D(numSample+1:1:2*numSample) = last_LS_D;
                    LS_D(numSample*2+1:1:3*numSample) = last_last_LS_D;
                end
                last_LS_D = InphaseSignal';
            ...
            if enable_RLS
                ...
                for subindex = 1 : corr_per_chip_plot*2+1
                    ...
                    %for ind = 1 : corr_per_chip_plot*2+1
                    %    for subindex = 1 : corr_per_chip_plot*2+1
                    %        d(1,subindex) = sum(code_replicas_plot(subindex,:).*code_replicas_plot(ind,:));
                    %    end
                ...
                %%%%%%%% INVERSE CHANNEL %%%%%%%%
                [channel3] = filter_rls2(u, d, lambda, channel3);
                ...
                for subindex = 1 : corr_per_chip_plot*2+1
                    f_corr_out(1,subindex) = sum(code_replicas_plot(subindex,:).*code_replicas_plot(corr_per_chip_plot+1+t_los-2,:));
                        %f_corr_out(1,subindex) = sum(code_replicas_plot(subindex,:).*code_replicas_plot(t_los,:));
                end
    ...
    function [h] = filter_rls2(u, d, lambda, h)
