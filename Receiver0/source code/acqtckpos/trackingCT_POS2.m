function [TckResultCT, navSolutionsCT] = trackingCT_POS(file,signal,track,cmn, Acquired,cnslxyz,eph,sbf)
    ...
    for msIndex = 1: 2000/pdi 
        ...
        for svindex = 1 :svlength
            ...
            if enable_RLS
                ...
                for ind = 1 : total_corr
                    for subindex = 1 : total_corr
                        d(1,subindex) = sum(code_replicas_plot(subindex,:).*code_replicas_plot(ind,:));
                    end
                    rmse(1,ind) = sqrt(mean((u - d).^2));
                end
                [a_los1, t_los1] = min(rmse);
                for subindex = 1 : total_corr
                    f_corr_out(1,subindex) = sum(code_replicas_plot(subindex,:).*code_replicas_plot(t_los1,:));
                end
                %}
                ...
                %%%%%%%% REMOVE MULTIPATH CONTRIBUTION %%%%%%%%
                
                new_corr_out = abs(corr_out_plot);
                for subindex = 1 : size(channel3(:,1))
                    if subindex ~= t_los
                        for subindex1 = 1 : total_corr 
                            dummy_corr_out(1,subindex1) = sum(code_replicas_plot(subindex1,:).*code_replicas_plot(one_side_corr+1+subindex-2,:));
                        end
                        new_corr_out = new_corr_out - channel3(subindex,1).*dummy_corr_out;
                    end
                end
                ...
                E_i = f_corr_out(1,one_side_corr-1); 
                L_i = f_corr_out(1,one_side_corr+3);
    ...
    function [h] = filter_rls3(u, d, lambda, h)
