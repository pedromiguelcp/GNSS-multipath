function [TckResultCT_pos, navSolutionsCT] = trackingCT_POS_updated(file,signal,track,cmn, Acquired,TckResult_Eph, cnslxyz,eph,sbf,solu)
    ...
    %%%%%%%% RLS VARS %%%%%%%%
    corr_per_chip_plot = 20; % correlators per chip
    one_side_chip = 1; % total correlator spacing (e.g. -1 to 1, -1.5 to 1.5)
    ...
    P_rls = zeros(total_corr,total_corr,svlength);
    for Index = 1: svlength
        P_rls(:,:,Index) = eye(total_corr)/0.005;
    end
    ch_imp_resp =zeros(total_corr,svlength);
    ...
    enable_RLS = 1;
    enable_RLS_compensation_plot = 1;
    ...
    for msIndex = 1: datalength/pdi % Note that for pdi > 1ms, the index is still denoted as msIndex. 30/04/2020, BING XU
        ...
        for svindex = 1 :svlength
            ...
            if enable_RLS & (msIndex > 100) % wait for tracking pull-in
                ...
                %{ 
                    %%%%%%%%%%%%%%%% HRC - only compute VE, E, P, L, and VL %%%%%%%%%%%%%%%%
                IE2=corr_inphase(one_side_corr-1);
                IE1=corr_inphase(one_side_corr);
                IP=corr_inphase(one_side_corr+1);
                IL1=corr_inphase(one_side_corr+2);
                IL2=corr_inphase(one_side_corr+3);
                QE2=corr_quadrat(one_side_corr-1);
                QE1=corr_quadrat(one_side_corr);
                QP=corr_quadrat(one_side_corr+1);
                QL1=corr_quadrat(one_side_corr+2);
                QL2=corr_quadrat(one_side_corr+3);
                if (msIndex > 100)
                    IDhrc=(IE1-IL1)-(IE2-IL2)/2;
                    QDhrc=(QE1-QL1)-(QE2-QL2)/2;
                    SP=IP^2+QP^2;
                    codeError(svindex) = (IDhrc*IP + QDhrc*QP)/SP;
                else
                    E = sqrt(E_i^2+E_q^2);
                    L = sqrt(L_i^2+L_q^2);
    ...
    %}
               ...
               %}
                ...
                %{
                    %%%%%%%%%%%%%%%% Least-Squares %%%%%%%%%%%%%%%%
                LS_G = zeros(numSample, total_corr);
                for subindex = 1: total_corr
                    LS_G(:, subindex) = code_replicas_plot(subindex,:);
                end
                LS_D = InphaseSignal';
                LS_H = abs(pinv(LS_G'*LS_G)*LS_G'*LS_D);
                [a0, t0] = max(LS_H);
                %figure;
                %plot(Spacing_plot(1 : 1 : total_corr),LS_H(1 : 1 : total_corr)/max(LS_H),'-','Color',"#D95319",'DisplayName','Least Squares');
                if (msIndex > 100)
                    if t0 == one_side_corr+1
                        E = numSample-numSample/one_side_corr;
                        L = numSample-numSample/one_side_corr;
                    elseif t0 < one_side_corr+1
                        E = numSample;
                        L = numSample-numSample/one_side_corr;
                    else
                        E = numSample-numSample/one_side_corr;
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                % (A) below are different techniques trying to integrate the RLS to the tracking.
                % uncommented (lines 344-417) are 2 different tehniques that remove the contribution of the multipath 
                % the contribution of the multipath from the correlation function (in 2 different ways)
                %
                % (B) commented (lines 422-458) is another technique that only looks to the max impulse and produces a new
                % clean correlation function with the peak shifted according to the estimated LOS
                %
                % select (A) or (B) using the block comment as shown
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                % ~ same as
                %d = [linspace(0,numSample,one_side_corr+1) linspace(numSample-numSample/one_side_corr,0,one_side_corr)];
                ...
                for multp_num = 1 : 40
                    ...
                    rmse(1,multp_num) = sqrt(mean((u(1:1:end) - x_corr_out(1:1:end)).^2)); % compare against real correlation function
                end
                [a0, t0] = min(rmse);
                
                %%%%%%%% MULTIPATH MITIGATION 1 %%%%%%%%
                % removes the interference from the correlation function by producing a correlation function from
                % all impulses except the LOS and subtracting to the current correlation function
                ...
                % produces one correlation function for each impulse except the LOS and subtractes to the current correlation function
                ...
                %E_i = new_corr(one_side_corr); % new correlator values for DLL corrections
                %L_i = new_corr(one_side_corr+2);
                %E_i = new_corr1(one_side_corr); % new correlator values for DLL corrections
                %L_i = new_corr1(one_side_corr+2);
    
                %%%%%%%% PLOT %%%%%%%%
                if (svindex == 8) & enable_RLS_compensation_plot
                ...
                % from the LOS position in the impulse response, produce a correlation function
                % that will tell which correction the correlators need. The correlation peak can be at the
                % prompt correlator, late or early
                for subindex = 1 : total_corr
