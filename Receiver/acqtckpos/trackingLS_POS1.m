function [TckResultCT_pos, navSolutionsCT] = trackingCT_POS_updated(file,signal,track,cmn, Acquired,TckResult_Eph, cnslxyz,eph,sbf,solu)
    ...
    for msIndex = 1: datalength/pdi % Note that for pdi > 1ms, the index is still denoted as msIndex. 30/04/2020, BING XU
        ...
        for svindex = 1 :svlength
            ...
            %%%%%%%% RLS TRACKING %%%%%%%%
            if enable_LS & (msIndex > 100) % wait for tracking pull-in
                ...
                if msIndex == 101 % same for all sv
                    virtualFS=26000000;
                    SpacingLS = [(-1:0.04:1)];
                    total_corrLS = size(SpacingLS,2);
                    codePhaseStepLS(svindex) = codeFreq(svindex)/virtualFS;
                    numSampleLS = ceil((signal.codelength*pdi-remChip(svindex))/codePhaseStepLS(svindex));
                    time_stampsLS = zeros(total_corrLS,numSampleLS);
                    code_replicasLS = zeros(total_corrLS,numSampleLS);
                    
                    for subindex = 1: total_corrLS
                        time_stampsLS(subindex,:) = (SpacingLS(subindex)) : codeFreq(svindex)/virtualFS : ((numSampleLS -1) * (codeFreq(svindex)/virtualFS) + SpacingLS(subindex));
                        code_replicasLS(subindex,:) = Code_plot(svindex,ceil(time_stampsLS(subindex,:)) + 7);
                    end
                    LS_G = zeros(total_corrLS, total_corrLS);
                    for subindex = 1 : total_corrLS
                        for subindex1 = 1 : total_corrLS 
                            LS_G(subindex1,subindex) = sum(code_replicasLS(subindex,:).*code_replicasLS(subindex1,:));
                        end
                    end
                    LS_S=pinv(LS_G'*LS_G)*LS_G';
                end
                
                LS_D = abs(corr_out_plot)';
                ...
                % Number of points in the interpolated vector
                numPointsInterpolated = 101;
                
                % Original vector indices
                originalIndices = 0:numel(originalVector)-1;
                
                % Interpolated vector indices
                interpolatedIndices = linspace(originalIndices(1), originalIndices(end), numPointsInterpolated);
                
                % Interpolated vector using linear interpolation
                interpolatedVector = interp1(originalIndices, originalVector, interpolatedIndices, 'linear');
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %}


                LS_H = LS_S*LS_D;
                %LS_H = LS_S*interpolatedVector';
                [a_los, t_los] = max(LS_H);
            end

            
            % Implement code loop filter and generate NCO command
            if enable_LS & (msIndex > 100)
                if (t_los==one_side_corr+1)
                    E = corr_out_plot(one_side_corr);
                    L = corr_out_plot(one_side_corr+2);
                    ...
                    %codeError(svindex) = (SpacingLS(51)-SpacingLS(t_los));
                end
            else
                E = sqrt(E_i^2+E_q^2);
                L = sqrt(L_i^2+L_q^2);
